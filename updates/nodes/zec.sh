...
echo
echo "Downloading Zcash v2.0.5-2..."
curl -#Lo /tmp/zcash.tar.gz https://z.cash/downloads/zcash-2.0.5-2-linux64.tar.gz >> ${LOG_FILE} 2>&1
...
cp /tmp/zcash-2.0.5-2/bin/* /usr/local/bin/ >> ${LOG_FILE} 2>&1
rm -r /tmp/zcash-2.0.5-2 >> ${LOG_FILE} 2>&1
