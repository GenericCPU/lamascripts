...
echo
echo "Downloading Zcash v2.0.7..."
curl -#Lo /tmp/zcash.tar.gz https://z.cash/downloads/zcash-2.0.7-linux64-debian-jessie.tar.gz >> ${LOG_FILE} 2>&1
...
cp /tmp/zcash-2.0.7/bin/* /usr/local/bin/ >> ${LOG_FILE} 2>&1
rm -r /tmp/zcash-2.0.7 >> ${LOG_FILE} 2>&1
